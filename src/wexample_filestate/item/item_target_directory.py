...
class ItemTargetDirectory(ItemDirectoryMixin, AbstractItemTarget):
    ...
    def _build_operations_parallel(
        ...
        # thread is on. Lines from concurrent workers interleave — single-line
        # writes to stdout are atomic on Linux up to PIPE_BUF, so each line
        # stays whole. The counter is the start ordinal (itertools.count is
        # thread-safe in CPython), so numbers may appear out of order on screen
