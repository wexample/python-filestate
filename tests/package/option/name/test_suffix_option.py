...
class TestSuffixOption(AbstractStateManagerTest):
    ...
    def test_suffix_option_validate_name_file_extension(self, tmp_path) -> None:
        ...
        assert option.validate_name("file.txt.bak") is False, "Should not match file.txt.bak"
        assert option.validate_name("txt") is False, "Should not match txt (no dot)"
        assert option.validate_name("file") is False, "Should not match file (no extension)"
    ...
    def test_suffix_option_validate_name_custom_suffix(self, tmp_path) -> None:
        ...
        assert option.validate_name("backup_file") is False, "Should not match backup_file"
        assert option.validate_name("file_backup_old") is False, "Should not match file_backup_old"
        assert option.validate_name("backup") is False, "Should not match backup (no underscore)"
    ...
    def test_suffix_option_validate_name_case_sensitive(self, tmp_path) -> None:
        ...
        assert option.validate_name("file.txt") is False, "Should not match file.txt (lowercase)"
        assert option.validate_name("file.Txt") is False, "Should not match file.Txt (mixed case)"
    ...
    def test_suffix_option_apply_correction_add_suffix(self, tmp_path) -> None:
        ...
        assert option.apply_correction("document") == "document.txt", "Should add .txt to document"
        assert option.apply_correction("test_file") == "test_file.txt", "Should add .txt to test_file"
        ...
        assert option.apply_correction("document.txt") == "document.txt", "Should not modify document.txt"
    ...
    def test_suffix_option_apply_correction_custom_suffix(self, tmp_path) -> None:
        ...
        # Should add suffix to names that don't have it
        assert option.apply_correction("file") == "file_backup", "Should add _backup to file"
        assert option.apply_correction("data") == "data_backup", "Should add _backup to data"
        assert option.apply_correction("test_file") == "test_file_backup", "Should add _backup to test_file"
        
        # Should not modify names that already have the suffix
        assert option.apply_correction("file_backup") == "file_backup", "Should not modify file_backup"
        assert option.apply_correction("data_backup") == "data_backup", "Should not modify data_backup"
    ...
    def test_suffix_option_apply_correction_case_sensitive(self, tmp_path) -> None:
        ...
        assert option.apply_correction("file.txt") == "file.txt.TXT", "Should add .TXT even if .txt exists"
        assert option.apply_correction("file.Txt") == "file.Txt.TXT", "Should add .TXT even if .Txt exists"
    ...
    def test_suffix_option_apply_correction_multiple_suffixes(self, tmp_path) -> None:
        ...
        assert option.apply_correction("file.txt") == "file.txt.bak", "Should add .bak to file.txt"
        assert option.apply_correction("script.py") == "script.py.bak", "Should add .bak to script.py"
        assert option.apply_correction("data.json.old") == "data.json.old.bak", "Should add .bak to data.json.old"
        
        # Should not modify if already ends with .bak
        assert option.apply_correction("file.txt.bak") == "file.txt.bak", "Should not modify file.txt.bak"
    ...
    def test_suffix_option_special_characters_in_suffix(self, tmp_path) -> None:
        ...
        assert option.validate_name("app_v1.0-final") is True, "Should match app_v1.0-final"
        assert option.validate_name("lib_v1.0-final") is True, "Should match lib_v1.0-final"
        ...
        assert option.validate_name("app_v1.0-beta") is False, "Should not match app_v1.0-beta"
        
        # Should add the special suffix when correcting
        assert option.apply_correction("app") == "app_v1.0-final", "Should add special suffix to app"
        assert option.apply_correction("lib") == "lib_v1.0-final", "Should add special suffix to lib"
