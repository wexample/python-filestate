...
class TestRegexOption(AbstractStateManagerTest):
    ...
    def test_regex_option_creation(self, tmp_path) -> None:
        ...
        assert option.get_description() == "Enforce regex pattern matching for file names"
    ...
    def test_regex_option_validate_name_file_extension_pattern(self, tmp_path) -> None:
        ...
        assert option.validate_name("script.txt") is False, "Should not match script.txt"
        assert option.validate_name("file.pyc") is False, "Should not match file.pyc"
        assert option.validate_name("python") is False, "Should not match python (no extension)"
    ...
    def test_regex_option_validate_name_complex_pattern(self, tmp_path) -> None:
        ...
        assert option.validate_name("test_file_123.txt") is True, "Should match test_file_123.txt"
        assert option.validate_name("my_script_001.py") is True, "Should match my_script_001.py"
        assert option.validate_name("data_model_42.json") is True, "Should match data_model_42.json"
        
        # Should not match the pattern
        assert option.validate_name("TestFile123.txt") is False, "Should not match TestFile123.txt (uppercase)"
        assert option.validate_name("test_file.txt") is False, "Should not match test_file.txt (no number)"
        assert option.validate_name("test_file_abc.txt") is False, "Should not match test_file_abc.txt (letters instead of number)"
        assert option.validate_name("test_file_123") is False, "Should not match test_file_123 (no extension)"
    
    def test_regex_option_validate_name_case_insensitive_pattern(self, tmp_path) -> None:
        ...
        assert option.validate_name("INSTALL.md") is False, "Should not match INSTALL.md"
        assert option.validate_name("my_readme.txt") is False, "Should not match my_readme.txt"
    ...
    def test_regex_option_apply_correction_no_change(self, tmp_path) -> None:
        ...
        assert option.apply_correction("invalid_name.txt") == "invalid_name.txt", "Should return name unchanged"
        assert option.apply_correction("test_file.txt") == "test_file.txt", "Should return name unchanged"
    ...
    def test_regex_option_empty_pattern(self, tmp_path) -> None:
        ...
        assert option.validate_name("") is True, "Should match empty string with empty pattern"
        assert option.validate_name("test.txt") is True, "Should match any string with empty pattern"
    ...
    def test_regex_option_dot_star_pattern(self, tmp_path) -> None:
        ...
        assert option.validate_name("file with spaces.doc") is True, "Should match file with spaces"
        assert option.validate_name("special!@#$%^&*()chars.txt") is True, "Should match special characters"
    ...
    def test_regex_option_anchored_vs_unanchored_patterns(self, tmp_path) -> None:
        ...
        assert option_unanchored.validate_name("test_file") is True, "Should match 'test_file' (starts with test)"
        # re.match() only matches from the beginning, so these will fail
        assert option_unanchored.validate_name("my_test") is False, "Should not match 'my_test' (re.match only matches from start)"
        assert option_unanchored.validate_name("my_test_file") is False, "Should not match 'my_test_file' (re.match only matches from start)"
        ...
        assert option_anchored.validate_name("test_file") is True, "Should match 'test_file' (starts with test)"
        assert option_anchored.validate_name("my_test") is False, "Should not match 'my_test' (doesn't start with test)"
        assert option_anchored.validate_name("my_test_file") is False, "Should not match 'my_test_file' (doesn't start with test)"
