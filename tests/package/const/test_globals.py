    ...
    def test_patterns_with_real_files(self, tmp_path) -> None:
        ...
        for item in tmp_path.iterdir():
            ...
            if item_name.startswith('.'):
                # Hidden items
                assert any_pattern.match(item_name), f"ANY_ITEM should match {item_name}"
                assert not no_dot_pattern.match(item_name), f"NO_LEADING_DOT should not match {item_name}"
            else:
                # Normal items
                assert any_pattern.match(item_name), f"ANY_ITEM should match {item_name}"
                assert no_dot_pattern.match(item_name), f"NO_LEADING_DOT should match {item_name}"
